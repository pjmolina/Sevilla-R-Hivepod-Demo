...
geojson(OficinaSchema, {
  type: 'Point',
  path: 'localizacion',
  required: true
});

OficinaSchema.index({'localizacion': '2dsphere'});
...
var WeatherSchema = new mongoose.Schema({
	...
	relHum: { type: Number, required: true },
	presure: { type: Number, required: true },
...
var ExoplanetSchema = new mongoose.Schema({
	...
	firstRef: { type: String, required: true }
});
...
var propertiesForClass = {
	...
	"oficina" : ['nombre', 'localizacion', 'telefono', 'imagen'],
...
buildModelAndControllerForSchema(models, 'olympicMedal', 'olympicMedals', OlympicMedalSchema);
buildModelAndControllerForSchema(models, 'oficina', 'oficinas', OficinaSchema);
buildModelAndControllerForSchema(models, 'weather', 'weathers', WeatherSchema);
buildModelAndControllerForSchema(models, 'exoplanet', 'exoplanets', ExoplanetSchema);
