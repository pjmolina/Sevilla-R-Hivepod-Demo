angular.module('myApp').controller('EditWeatherController', 
  ['$scope', '$routeParams', '$location', '$translate', '$timeout', 'UserErrorService', 'NavigationService', 'EntityUtilService', 'SecurityService', 'WeatherService', 
  function($scope, $routeParams, $location, $translate, $timeout, UserErrorService, NavigationService, EntityUtilService, SecurityService, WeatherService) {
	...
	$scope.add = function () {
		...
		WeatherService.add(dataToServer($scope.obj)).then(function (httpResponse) {
			if($scope.parent) {
	...
	$scope.update = function () {
		$scope.uiWorking = true;
		WeatherService.update(dataToServer($scope.obj)).then(function () { //httpResponse
				returnBack();
	...
	$scope.delete = function () {
		$scope.uiWorking = true;
		WeatherService.delete($scope.obj._id).then(returnBack, errorHandlerDelete, progressNotify);
	};
	...
	$scope.gotoEdit = function() {
		$location.path('/weather/edit/' + $routeParams.id);		
	};

	$scope.gotoDelete = function() {
		$location.path('/weather/delete/' + $routeParams.id);		
	};
	...
	function init() {
		...
		if ($routeParams.id) {
			WeatherService.getDocument($routeParams.id).then(loadData, errorHandlerLoad);		
		}
