angular.module('myApp').controller('EditExoplanetController', 
  ['$scope', '$routeParams', '$location', '$translate', '$timeout', 'UserErrorService', 'NavigationService', 'EntityUtilService', 'SecurityService', 'ExoplanetService', 
  function($scope, $routeParams, $location, $translate, $timeout, UserErrorService, NavigationService, EntityUtilService, SecurityService, ExoplanetService) {
	...
	$scope.add = function () {
		...
		ExoplanetService.add(dataToServer($scope.obj)).then(function (httpResponse) {
			if($scope.parent) {
	...
	$scope.update = function () {
		$scope.uiWorking = true;
		ExoplanetService.update(dataToServer($scope.obj)).then(function () { //httpResponse
				returnBack();
	...
	$scope.delete = function () {
		$scope.uiWorking = true;
		ExoplanetService.delete($scope.obj._id).then(returnBack, errorHandlerDelete, progressNotify);
	};
	...
	$scope.gotoEdit = function() {
		$location.path('/exoplanet/edit/' + $routeParams.id);		
	};

	$scope.gotoDelete = function() {
		$location.path('/exoplanet/delete/' + $routeParams.id);		
	};
	...
	function init() {
		...
		if ($routeParams.id) {
			ExoplanetService.getDocument($routeParams.id).then(loadData, errorHandlerLoad);		
		}
