angular.module('myApp').controller('ListExoplanetController', 
  ['$http', '$scope', '$location', '$q', '$timeout', '$modal', 'SecurityService', 'NavigationService', 'EntityUtilService', 'ExoplanetService', 
  function($http, $scope, $location, $q, $timeout, $modal, SecurityService, NavigationService, EntityUtilService, ExoplanetService) {
	...
	$scope.add = function () {
		$location.path('/exoplanet/add');
	};

	$scope.edit = function (obj) {
		$location.path('/exoplanet/edit/' + obj._id);
	};
	$scope.view = function (obj) {
		$location.path('/exoplanet/view/' + obj._id);
	};
	$scope.delete = function (obj) {
		$location.path('/exoplanet/delete/' + obj._id);
	};
	...
	function doDeleteByQuery() {
		...
		return EntityUtilService.deleteByQuery(ExoplanetService, searchCriteria, $scope.refresh);  		
	}
	...
	function doDeleteSelected() {
		EntityUtilService.deleteSelected(ExoplanetService, $scope.dataList, $scope.refresh);  
	}
	...
	$scope.importData = function () {
		...
		$location.path('/import/exoplanet');		
	};
	$scope.exportAs = function (format) { 
		EntityUtilService.exportAsFormat(
			...
			ExoplanetService, "exoplanets", $scope);
		$scope.ui.dropdown.isOpen = false;
	};
	//-----------------------------
	...
	function init() {
		SecurityService.getPermisionsFoResource('exoplanet').then(function(httpData) {
